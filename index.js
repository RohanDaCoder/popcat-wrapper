...
class CodeClient {
  ...
  async createBin({ title, description, code, theme, language }) {
    ...
    if (!code) throw new Error("[Popcat Wrapper] CodeClient.createBin(...) => 'code' parameter is required.");

    if (language && !allowedLanguages.some(l => l.toLowerCase() === language.toLowerCase())) {
    ...
    if (!json || json.error) {
      throw new Error(`[Popcat Wrapper] CodeClient.createBin(...) => ${json?.error || "Unknown error occurred"}`);
    }
...
module.exports.discordMessage = ({ username, content, avatar, color, timestamp }) => {
  if (!username) throw new Error("[Popcat Wrapper] discordMessage({ username, content, avatar, color, timestamp }) => 'username' parameter is missing.");
  if (!content) throw new Error("[Popcat Wrapper] discordMessage({ username, content, avatar, color, timestamp }) => 'content' parameter is missing.");
...
module.exports.couldread = (text) => {
  if (!text || text.length === 0) throw new Error("[Popcat Wrapper] couldread(text) => 'text' parameter is missing.");
  const url = `${baseurl}v2/couldread?text=${encodeURIComponent(text)}`;
  return url;
};

module.exports.supreme = (text) => {
  if (!text || text.length === 0) throw new Error("[Popcat Wrapper] supreme(text) => 'text' parameter is missing.");
...
module.exports.happysad = async (text1, text2) => {
  if (!text1 || !text2) throw new Error("[Popcat Wrapper] happysad(text1, text2) => Either 'text1' or 'text2' parameter is missing.");
...
module.exports.communism = async (imageURL) => {
  if (!imageURL) throw new Error("[Popcat Wrapper] communism(image) => 'imageURL' parameter is missing.");
  const url = `${baseurl}v2/communism?image=${encodeURIComponent(imageURL)}`
  return url;
}

module.exports.randomcolor = async () => {
  const color = await fetch("https://api.popcat.xyz/v2/randomcolor").then(r => r.json());
  return color;
}
...
module.exports.periodicTable = async (element) => {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/periodic-table?element=${encodeURIComponent(element)}`);
  const { error, message } = await res.json();
  if (error) throw new Error(`[Popcat Wrapper] Periodic Table: ${message.error}`);
  return message;
};
...
module.exports.imdb = async function (name) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/imdb?q=${encodeURIComponent(name)}`).then(r => r.json())
...
module.exports.steam = async function (name) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/steam?q=${encodeURIComponent(name)}`)
  const { error, message } = await res.json();
...
module.exports.screenshot = async function (url) {
  ...
  const { isurl } = await fetch(`https://api.popcat.xyz/v2/is-url?url=${encodeURIComponent(url)}`).then(r => r.json())
...
module.exports.lyrics = async function (song) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/lyrics?song=${encodeURIComponent(song)}`)
  const { error, message } = await res.json()
...
module.exports.subreddit = async function (subreddit) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/subreddit/${encodeURIComponent(subreddit)}`)
  const { error, message } = await res.json();
  if (error) throw new Error(`[Popcat Wrapper] ${message.error}`)
  return message;
}
...
module.exports.github = async function (username) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/github/${encodeURIComponent(username)}`);
  const { error, message } = await res.json();
...
module.exports.weather = async function (place) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/weather?q=${encodeURIComponent(place)}`)
  const { error, message } = await res.json();
...
module.exports.country = async function (name) {
  ...
  const { error, message } = await res.json();
  if (error) throw new Error(`[Popcat Wrapper] Invalid country in the COUNTRY function!`);
  return message;
}
...
module.exports.reverse = async function (text) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/reverse?text=${encodeURIComponent(text)}`)
  const { error, message } = await res.json();
...
module.exports.colorinfo = async function (color) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/color/${encodeURIComponent(colour)}`);
  const { error, message } = await res.json();
  if (error) throw new Error(`[Popcat Wrapper] Invalid hex in the 'colorinfo' function!`);
  return message;
}
...
module.exports.wouldyourather = async function () {
  ...
  const { error, message } = await res.json();
  if (error) throw new Error(`[Popcat Wrapper] WouldYouRather: ${message.error}`)
  return message;
}
...
module.exports.welcomecard = function welcomecard(background, avatar, text_1, text_2, text_3) {
...
module.exports.itunes = async function (x) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/itunes?q=${encodeURIComponent(x)}`);
  const { error, message } = await res.json();
...
module.exports.chatbot = async function (x, ownername, botname) {
  ...
  const res = await fetch(`https://api.popcat.xyz/v2/chatbot?msg=${encodeURIComponent(x)}&owner=${encodeURIComponent(ownername)}&botname=${encodeURIComponent(botname)}`)
  const { error, message } = await res.json();
...
module.exports.encode = async function (text) {
  ...
  const { error, message } = await res.json();
  if (error) throw new Error(`[Popcat Wrapper] Binary Encode: ${message.error}`)
  return message.text;
}
module.exports.decode = async function (text) {
  ...
  const { error, message } = await res.json();
  if (error) throw new Error(`[Popcat Wrapper] Binary Decode: ${message.error}`)
  return message.text;
}
